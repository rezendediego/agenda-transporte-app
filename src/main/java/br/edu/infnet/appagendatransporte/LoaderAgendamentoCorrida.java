		...
		while (linha != null) {
			...
			case "D":
				Delivery delivery = new Delivery(campos[1],
						LocalDateTime.parse(campos[2], formatadorDataHora),
						new Endereco(campos[3], Integer.valueOf(campos[4]), campos[5]),
						new Endereco(campos[6], Integer.valueOf(campos[7]), campos[8]), Integer.valueOf(campos[9]),
						TipoProduto.valueOf(campos[10]), Boolean.valueOf(campos[11]), campos[12]);
				
				agendamentoCorrida.getListaCorridaAgendada().add(delivery);
				break;

			case "P":
				Pet pet = new Pet(campos[1], LocalDateTime.parse(campos[2], formatadorDataHora),
						new Endereco(campos[3], Integer.valueOf(campos[4]), campos[5]),
						new Endereco(campos[6], Integer.valueOf(campos[7]), campos[8]), Integer.valueOf(campos[9]),
						TipoAnimal.valueOf(campos[10]), Boolean.valueOf(campos[11]), Boolean.valueOf(campos[12]));
		
				agendamentoCorrida.getListaCorridaAgendada().add(pet);
				break;

			case "V":
				Viagem viagem = new Viagem(campos[1], LocalDateTime.parse(campos[2], formatadorDataHora),
						new Endereco(campos[3], Integer.valueOf(campos[4]), campos[5]),
						new Endereco(campos[6], Integer.valueOf(campos[7]), campos[8]), Integer.valueOf(campos[9]),
						Boolean.valueOf(campos[10]), Boolean.valueOf(campos[11]), DistanciaViagem.valueOf(campos[12]));
